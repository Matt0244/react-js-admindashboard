...
export default function ProdectAddUpdate() {
  ...
  const handleChange = async ({file, fileList  }) =>{ 
    console.log(fileList); // 打印新的 fileList
    ...
    return new Promise((resolve, reject) => {
      Modal.confirm({
        title: "Are you sure you want to delete this file?",
         onOk: () => {
        // Send an API request to delete the image
        reqDeleteImg(file.name)
          .then(() => {
            // If the request is successful, resolve the Promise with true
            resolve(true);
          })
          .catch(() => {
  ...
  const { name, desc, price,imgs, _id} = record;
  console.log(imgs)
  console.log(record)
  ...
  const onChange = (value, selectedOptions) => {
    console.log("我是onChange" + value, selectedOptions);
    setParentId(value);
  };
  ...
  return (
    ...
    >
      ...
      >
        <TextArea rows={4} defaultValue={desc || ""} />
      </Form.Item>
      ...
      >
        <Input type="number" addonAfter="元" defaultValue={price || 0} />
      </Form.Item>
      ...
      <Form.Item label="图片上传" name="imgs">
        {/* 渲染上载图片的modol  这里的action就是发送post的路径  name 是发送的名字*/}
        <Upload
          action="/manage/img/upload" /*上传图片的接口地址*/
          accept="image/*" /*只接收图片格式*/
          name="image" 
          listType="picture"
          fileList={fileList} // 这里显示的是图片的名字
